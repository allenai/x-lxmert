...
if __name__ == '__main__':
    ...
    parser.add_argument('--vgroot', type=str, default='../datasets/VG/')
    ...
    vg_dir = Path(args.vgroot).resolve()
    dataset = VGDataset(vg_dir)
    dataloader = DataLoader(dataset, batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_fn, num_workers=4)

    out_dir = vgroot.joinpath('features').resolve()
    if not out_dir.exists():
        out_dir.mkdir()
    output_fname = out_dir.joinpath(f'{args.backbone}_boxes{NUM_FEATURES}.h5')
    if args.test:
        output_fname = out_dir.joinpath(f'{args.backbone}_boxes{NUM_FEATURES}_test.h5')
    print('Output features will be saved at', output_fname)

    with h5py.File(output_fname, 'w') as f:
        with torch.no_grad():
            for i, batch in tqdm(enumerate(dataloader),
                                 desc=f'{args.backbone}_{(NUM_FEATURES, dim)}', ncols=150,
                ...
                for img_id, feat, info in zip(img_ids, feat_list, info_list):
                    ...
                    assert feat.shape == (NUM_FEATURES, dim), ('feat', feat.shape)
                    assert info['objects'].shape == (NUM_FEATURES,), ('obj_ids', info['objects'].shape)
                    assert info['bbox'].shape == (NUM_FEATURES, 4), ('bbox', info['bbox'].shape)

                    # boxes = deepcopy(info['bbox']) # (N, 4) xyxy
                    ...
                    # # print('max', boxes.max())
                    # # print('min', boxes.min())
                    # np.testing.assert_array_less(boxes, 1+2e-1)
